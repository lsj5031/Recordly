...
import type { ExportConfig, ExportEncodingInfo, ExportProgress, ExportResult } from "./types";
...
export class VideoExporter {
	...
	private resolvedHardwareAcceleration: "prefer-hardware" | "prefer-software" | null = null;
	...
	async export(): Promise<ExportResult> {
		try {
			...
			return { success: true, blob, encoding: this.getEncodingInfo() };
		} catch (error) {
	...
	private getCodecFamily(codec: string | null): ExportEncodingInfo["codecFamily"] {
		if (!codec) {
			return "unknown";
		}

		if (codec.startsWith("avc1") || codec.startsWith("avc3") || codec.startsWith("h264")) {
			return "h264";
		}

		if (codec.startsWith("hvc1") || codec.startsWith("hev1") || codec.startsWith("hevc")) {
			return "hevc";
		}
	...
	private getEncodingInfo(): ExportEncodingInfo | undefined {
		if (!this.resolvedCodec || !this.resolvedHardwareAcceleration) {
			return undefined;
		}

		return {
			encoder: this.resolvedCodec,
			codecFamily: this.getCodecFamily(this.resolvedCodec),
			acceleration: this.resolvedHardwareAcceleration === "prefer-hardware" ? "webcodecs" : "cpu",
			hardwareAccelerated: this.resolvedHardwareAcceleration === "prefer-hardware",
		};
	}
	...
	private async initializeEncoder(frameRate: number): Promise<void> {
		...
		this.resolvedHardwareAcceleration = null;
		...
		for (const candidateCodec of CODEC_FALLBACK_LIST) {
			...
			if (hwSupport.supported) {
				...
				this.resolvedCodec = candidateCodec;
				this.resolvedHardwareAcceleration = "prefer-hardware";
				console.log(`[VideoExporter] Using hardware acceleration with codec ${candidateCodec}`);
				this.encoder.configure(hwConfig);
				return;
			}

			if (this.config.requireHardwareAcceleration) {
				console.warn(
					`[VideoExporter] Hardware encoding unavailable for codec ${candidateCodec}; software fallback disabled`,
				);
				continue;
			}
			...
			if (swSupport.supported) {
				resolvedCodec = candidateCodec;
				this.resolvedCodec = candidateCodec;
				this.resolvedHardwareAcceleration = "prefer-software";
		...
		if (this.config.requireHardwareAcceleration) {
			throw new Error(
				`Hardware-accelerated WebCodecs encoding is not available on this system. ` +
